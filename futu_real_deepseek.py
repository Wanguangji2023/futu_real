...
版本: v2.4-fixed
v2.4 修复清单:
- [严重] 持仓亏损时完全没有止损：原逻辑只有 check_level=="T0"(盈利≥5%)才调用
  _check_take_profit，0%~5% 盈利及亏损仓位永远不走止损/止盈检查。
  现改为只要持仓就检查，并新增硬止损（跌破 position_price*(1-stop_loss) 即清仓）。
- [严重] 动态止盈止损比例基于"当前盈利"计算，盈利回落时止损线反而放宽。
  改为基于"峰值盈利"计算止损比例。
- [严重] 钉钉推送失败时 logging.error 会再次触发 DingTalkHandler.emit，
  存在无限递归风险。增加推送中守卫，失败日志走独立 logger。
- [严重] quote 连接断开时无任何感知，程序空转。增加行情健康检查，
  异常时主动抛错触发外层自动重启。
- [中] get_stock_basicinfo 的 SDK 签名是 (market, stock_type)，不支持直接传
  股票代码列表，原调用必然失败。改为 get_market_snapshot 读取 lot_size。
- [中] 启动时每个 SymbolState 都 force 刷新一次持仓缓存（N 次网络请求）。
  改为 main 中统一刷新一次。
- [中] 账户选择未按市场过滤，多市场账户时可能选错。
- [中] 市价买入数量不足一手时被 max() 强制抬到一手，可能超出可用资金。
  改为不足一手直接放弃。
- [中] cooldown.csv 每次卖出追加一行，文件无限增长。改为 upsert。
- [中] 每个 tick 都全量重写 positions.csv，IO 压力大。增加 2 秒节流，
  成交/清仓时强制落盘。
- [中] 全局引擎锁导致单个标的下单时阻塞所有标的。改为按标的细粒度锁。
- [中] 黑名单/观察列表只在启动时加载，运行中其他标的加入观察后本标的缓存过期。
  增加 30 秒 TTL 自动刷新。
- [轻] 非交易时段唤醒前 busy-loop sleep(1)。改为非开盘时段 sleep(5)。
- [轻] 股票池外的本地持仓标的不会被监控。启动时自动纳入。
- [轻] 资金查看文件总市值始终为 0，用持仓缓存估算。
- [提示] US 夏令时按 UTC 日期判断，仅在 UTC 午夜边界有 1 小时误差，可接受。
- [提示] A股模拟盘市价单可能不被接受（需限价），A股已限定仅模拟盘，请注意。
FIX(v2.4-fixed):
1. 修复 FutuClient.get_acc_balance 接口：证券使用 get_funds()；移除错误 self.logger；兼容HK模拟盘avail_funds
2. 全部 datetime.utcnow() → datetime.now(UTC) 消除DeprecationWarning
3. 删除文件末尾错位重复全局函数，修复缩进BUG
4. 修复 get_next_market_open / wait_until_market_open 时间函数
功能：
- 模拟盘真实提交模拟订单
- A股仅允许模拟盘，禁止实盘
...
# 推送守卫：防止钉钉推送失败 -> logging.error -> 再次触发钉钉 handler 的递归
_dingtalk_guard = threading.local()
def push_to_dingtalk(webhook: str, secret: str = None, text: str = None) -> bool:
    if not webhook:
        return False
    if getattr(_dingtalk_guard, "pushing", False):
        return False
    try:
        _dingtalk_guard.pushing = True
        ...
        # 用独立 logger，绕过 root 上的 DingTalkHandler，杜绝递归
        logging.getLogger("notify.safe").warning(f"钉钉推送失败: {e}")
        return False
    finally:
        _dingtalk_guard.pushing = False
...
def save_cooldown_entry(inst_id: str, cooldown_until: str):
    ...
    for row in rows:
        if row.get("inst_id") == inst_id:
            row["sell_time"] = now_str
            row["cooldown_until"] = cooldown_until
            break
    else:
        rows.append({"inst_id": inst_id, "sell_time": now_str, "cooldown_until": cooldown_until})
    _write_csv_all(COOLDOWN_FILE, COOLDOWN_HEADER, rows)
...
class FutuClient:
    ...
    def _connect(self):
        ...
        trd_market_map = {"CN": ft.TrdMarket.CN, "HK": ft.TrdMarket.HK, "US": ft.TrdMarket.US}
        mk = trd_market_map.get(self.market)
        if mk is not None and "trd_market" in acc_df.columns:
            filtered = filtered[filtered["trd_market"] == mk]
        if filtered.empty:
            env_name = "模拟盘" if self.trd_env == ft.TrdEnv.SIMULATE else "实盘"
            raise FutuTradeError(f"未找到{env_name}账户(market={self.market})")
        ...
        self.acc_id = int(filtered.iloc[0]["acc_id"])
        logging.info(f"使用账户ID: {self.acc_id} ({'模拟' if self.trd_env == ft.TrdEnv.SIMULATE else '实盘'}, market={self.market})")
    ...
    #         # 证券OpenSecTradeContext 获取资金正确接口 get_account_info
    #         ret, data = self.trade_ctx.get_account_info()
    #         if ret != ft.RET_OK:
    #             raise Exception(f"get_account_info 接口错误 ret={ret}")
    #         if data.empty:
    #             raise Exception("get_account_info 返回空DataFrame")
    #         logging.info(f"账户资金返回列:{list(data.columns)}")
    #         # HK模拟盘/实盘字段
    #         if "avail_withdraw_cash" in data.columns:
    #             val = float(data.loc[0, "avail_withdraw_cash"])
    #         elif "avail_funds" in data.columns:
    #             val = float(data.loc[0, "avail_funds"])
    #         else:
    #             raise KeyError(f"资金字段缺失，可用列:{list(data.columns)}")
    #         return val
    def get_acc_balance(self):
        # 证券交易上下文正确接口 accinfo_query，必须传入 trd_env、acc_id
        ...
        logging.info(f"账户资金返回列:{list(data.columns)}")
        # 字段说明：
        # cash: 现金；avl_withdrawal_cash：可取现（模拟盘该字段为空）；power：购买力
        if "cash" in data.columns:
            val = float(data.loc[0, "cash"])
        elif "power" in data.columns:
            val = float(data.loc[0, "power"])
        else:
            raise KeyError(f"资金字段缺失，可用列:{list(data.columns)}")
        return val
    ...
    def _get_lot_size(self, code: str) -> int:
        ...
        try:
            ...
            if ret == ft.RET_OK and data is not None and not data.empty and "lot_size" in data.columns:
    ...
    def market_buy_amount(self, code: str, amount: float, ref_price: float = 0.0) -> Dict:
        ...
        if qty <= 0:
            return {"error": f"金额不足一手: {amount:.2f}/{price:.4g} < {lot_size}股"}
...
class SymbolState:
    def __init__(self, inst_id, ref_high, ref_low, system_params=None):
        ...
        self._lists_loaded_at = time.time()
    ...
    def _refresh_lists(self):
        now = time.time()
        if now - self._lists_loaded_at > 30:
            self.blacklist = load_blacklist()
            self.observe_list = load_observe_list()
    ...
    def _execute_buy(self, price, reason, buy_key, sell_key, pair_name):
        ...
        self.stop_price = price * (1 - self.params.get("stop_loss", 0.015))
    ...
    def _check_take_profit(self, price):
        """v2.4: 只要持仓就调用。包含：硬止损、20%清仓、动态止盈。"""
        if not self.has_position or self.position_price <= 0:
            return None
        stop_loss_pct = self.params.get("stop_loss", 0.015)
        profit_pct = (price - self.position_price) / self.position_price
        if price > self.peak_price:
            self.peak_price = price
        # 1) 硬止损：未触发止盈前，跌破成本价*(1-stop_loss) 无条件清仓
        if not self.profit_triggered and profit_pct <= -stop_loss_pct:
            logging.info(f"{self.inst_id} 触发止损清仓: 当前价 {price:.8g}, 跌幅 {-profit_pct*100:.2f}% ≥ {stop_loss_pct*100:.1f}%")
            return self._execute_sell(price, f"止损清仓 (跌幅 {-profit_pct*100:.2f}%)")
        # 2) 盈利≥20% 立即清仓
        if profit_pct >= 0.20:
            logging.info(f"{self.inst_id} 盈利≥20%立即清仓")
            return self._execute_sell(price, f"盈利率 {profit_pct*100:.1f}% ≥20%，清仓")
        # 3) 动态止盈：止损比例基于"峰值盈利"而非当前盈利，避免回落时止损放宽
        peak_profit_pct = (self.peak_price - self.position_price) / self.position_price
        stop_ratio = get_stop_loss(peak_profit_pct)
    ...
    def update_price(self, price, ts):
        ...
        if self.has_position:
            ...
            if res:
                alerts.append(f"止盈/止损清仓 @ {price:.8g}")
                self._save_state(force=True)
                return alerts
            if self.is_paired and not self.is_sold:
    ...
    def _save_state(self, force: bool = False):
        now = time.time()
        if not force and now - self._last_persist_time < 2.0:
            return
...
def _update_funds_for_view(balance: float):
    try:
        total_mv = 0.0
        total_upnl = 0.0
        for pp in load_positions().values():
            cost = float(pp.get("position_amount", 0) or 0)
            cp = float(pp.get("current_price", 0) or 0)
            qty = float(pp.get("position_qty", 0) or 0)
            mv = cp * qty if cp > 0 else cost
            total_mv += mv
            total_upnl += mv - cost
        row = {
            "remaining_funds": f"{balance:.2f}",
            "total_position_market_value": f"{total_mv:.2f}",
            "total_unrealized_pnl": f"{total_upnl:.2f}",
            "total_realized_pnl": "0",
            "net_asset_value": f"{balance + total_mv:.2f}",
...
class MarketEngine:
    ...
    def _lock_for(self, inst_id: str) -> threading.Lock:
        with self._locks_guard:
            return self._locks.setdefault(inst_id, threading.Lock())
    ...
    def on_tick(self, inst_id: str, price: float, ts: float):
        ...
        if alerts:
            key_alert = [a for a in alerts if any(k in a for k in ("止盈", "止损", "清仓", "买入", "卖出"))]
            if key_alert:
...
def main():
    ...
    try:
        ...
        return
    try:
        hl_store = load_high_low()
        for inst, p in load_positions().items():
            if inst not in config and p.get("position_qty", 0) > 0:
                hl = hl_store.get(inst, {})
                high = hl.get("all_time_high") or p.get("position_price") or 0
                low = hl.get("all_time_low") or p.get("position_price") or 0
                if high > 0 and low > 0:
                    config[inst] = {"ref_high": float(high), "ref_low": float(low)}
                    logging.info(f"持仓标的 {inst} 不在股票池中，已自动纳入监控")
    except Exception as e:
        logging.error(f"合并本地持仓到股票池失败: {e}")
    if not config:
    ...
    try:
        while True:
            ...
            any_open = any(is_market_open(m, utcnow) for m in markets_set)
            if now - last_check >= 60:
                if not any_open:
                    wait_until_market_open(list(markets_set))
                last_check = now
            if any_open and now - last_health >= 60:
                last_health = now
                try:
                    ret, _ = _futu_client.quote_ctx.get_market_snapshot([code_list[0]])
                    if ret != ft.RET_OK:
                        raise RuntimeError(f"行情快照请求失败 ret={ret}")
                except Exception as e:
                    logging.critical(f"行情健康检查失败，触发自动重启: {e}")
                    raise
            time.sleep(5 if not any_open else 1)
