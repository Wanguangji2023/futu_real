...
def get_market_timezone_offset(market, dt=None):
    ...
    if market in ('CN', 'HK'):
        ...
        # 注意：按 UTC 日期判断，UTC 午夜边界可能有 1 小时误差，可接受
...
class FutuClient:
    ...
    def _get_lot_size(self, code: str) -> int:
        ...
        lot = 0
        try:
            # v2.4: get_stock_basicinfo 签名是 (market, stock_type)，不支持传代码列表，
            # 原写法必然失败。改用快照读取 lot_size。
    ...
    def market_buy_amount(self, code: str, amount: float, ref_price: float = 0.0) -> Dict:
        ...
        # v2.4: 不足一手时放弃，不再强制抬到一手（避免超出可用资金）
...
class SymbolState:
    def __init__(self, inst_id, ref_high, ref_low, system_params=None):
        ...
        if self.client is not None:
            # v2.4: 不再每次 force 刷新（启动时 main 已统一刷新），避免 N 次网络请求
    ...
    def _refresh_lists(self):
        # v2.4: 黑名单/观察列表 30 秒 TTL 刷新，避免运行中缓存过期
        now = time.time()
    ...
    def update_price(self, price, ts):
        ...
        if self.has_position:
            ...
            # v2.4: 任何盈利水平都检查止损/止盈（原来只在 T0 检查，亏损仓位无保护）
    ...
    def _save_state(self, force: bool = False):
        # v2.4: 2秒节流，成交/清仓时由调用方传 force=True 强制落盘
        now = time.time()
...
def _update_funds_for_view(balance: float):
    try:
        total_mv = 0.0
        for p in get_api_positions().values():
            total_mv += float(p.get("position_amount", 0) or 0)
        row = {
...
def tick_callback(quote_ctx, ret_code, content):
    ...
    for item in content:
        ...
        if raw_time:
            try:
                # 注意：naive datetime.timestamp() 依赖运行机器的本机时区，请确保系统时区正确
...
def main():
    ...
    # v2.4: 本地有持仓但不在股票池中的标的自动纳入监控，避免持仓无人看管
    try:
        ...
        while True:
            ...
            if now - last_check >= 60:
                if not any_open:
                    wait_until_market_open(list(markets_set))
                last_check = now
            # v2.4: 行情健康检查。FutuOpenD 重启/断线后 tick 会静默停止，
            # 通过周期性快照探测连接，异常则抛错交给外层循环自动重启
